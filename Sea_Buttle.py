...
b1.place(x=size_canvas_x + menu_x / 8, y=60, width=menu_x / 4 * 3)  # отступ кнопки 1/8 от меню, ширина = 3/4 от меню
...
def draw_point(x, y):
    ...
    if points[y][x] > 1:  # Если в мвтрице POINTS по этим координатам уже есть id
        ...
        list_ids.append(id1)
        points[y][x] = id1
        print(f'id= {id1}, POINT= {points[y][x]}')
